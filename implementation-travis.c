...
//	if (size < sizeof(struct handle)) return NULL;
	...
	{
		s = size - sizeof(struct myfs_handle);
		...
		myHandle->freeMem = fsptr + 100000; //set freeMem ptr to point to a space far enough away from fsptr so that directory/file metadata doesn't segfault			
	}
	return myHandle;
}
...
/*
void init(void *fsptr, size_t size)
{
