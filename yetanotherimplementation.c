...
struct node {
	...
	struct timespec *lastAccess;          //file/dir last access time
    struct timespec *lastMod;          //file/dir last modified time
...
int __myfs_utimens_implem(void *fsptr, size_t fssize, int *errnoptr,
	...
	for (int i = 0; i < MAXFNUM; i++)
	{
		...
		{
    		memcpy(curr->lastAccess, &ts[0], sizeof(struct timespec));
    		memcpy(curr->lastMod, &ts[1], sizeof(struct timespec));
			return 0;
		}
		curr++;
	}
