...
int __myfs_getattr_implem(void *fsptr, size_t fssize, int *errnoptr,
	...
	const char *path, struct stat *stbuf) {
		...
		nd * curr = handle->rootDir;
		char * dup = strdup(path);
	//printf("size of fs = %ld", myHandle->size);
	...
	*/
	...
	for (int i = 0; i < MAXFNUM; i++)
	{
		if (strcmp(dup,curr->fullpath) == 0 && curr->fileSize == -1 && curr->isFree == -1)
		{
		...
		else if (strcmp(dup,curr->fullpath) == 0 && curr->isFree == -1 && curr->fileSize > -1)
		{
...
int __myfs_readdir_implem(void *fsptr, size_t fssize, int *errnoptr,
	const char *path, char ***namesptr) {
		...
		for (int i = 0; i < MAXFNUM; i++)
		{
			...
			//count how many subs there are
			if (strcmp(curr->path,dup) == 0)
			{
				subCount++;
			}
			curr++;
		...
		//reset curr
		curr = hand->rootDir;
		// dir not found or there are no subs
		if (dirFound == -1)
		{
			return -1;
		}
		...
		{
			// alloc n number of char pointers 
			temp = calloc(subCount,sizeof(char*));
			int d = 0;
			for (int i = 0; i < MAXFNUM; i++)
			{
				//subdir found
				...
				{
					// alloc space for sub
					temp[d] = calloc(sizeof(curr->name),sizeof(char));
					//set name 
					strcpy(temp[d],curr->name);
					d++;
				}
				curr++;
			}
...
int __myfs_truncate_implem(void *fsptr, size_t fssize, int *errnoptr,
	const char *path, off_t offset) {
		...
		{
			void * temp = searchLL(offset,hand);
			if (temp == NULL) return -1;
			memset(temp,0,offset);
			memcpy(temp,GETPTR(hand,curr->offset),offset);
			push(GETPTR(hand,curr->offset),hand);
			curr->offset = GETOFFSET(hand,temp);
			return 0;
		}
/* STUB */
}
...
int __myfs_read_implem(void *fsptr, size_t fssize, int *errnoptr,
	const char *path, char *buf, size_t size, off_t offset) {
		...
		for (int i = 0; i < MAXFNUM; i++)
		{
			if (strcmp(curr->fullpath,dup)==0 && curr->fileSize > 0)
			{
				found = 0;
				break;
			}
		...
		{
			if (offset > curr->fileSize)
			{
				return -1;
			}
			else
			{
				int sizeToRead = curr->fileSize - offset;
				memcpy(buf,GETPTR(hand,curr->offset),sizeToRead);
				return sizeToRead;
			}
			return -1;
		}
